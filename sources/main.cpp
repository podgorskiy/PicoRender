...
//
//			v = lambert_no_tangent(is.normal, rs);
//
//			if (k == 0)
//			{
//				albido = attenuation;
...
int main(int argc, char* argv[])
{
    ...
    optix::Context g_context = optix::Context::create();
    g_context->setRayTypeCount(1);
    g_context->setStackSize(3000);

    optix::Program rayGenAndBackgroundProgram = g_context->createProgramFromPTXString(render_program, "Render");
    ...
    g_context->launch(0, width, height);
    ...
    uint8_t* data8b = TOUINT8(pixels, 3 * width * height);
    std::string name("_out.png");
    stbi_write_png((outfile_prefix + name).c_str(), width, height, 3, data8b, 0);
    pixelBuffer->unmap();
