...
model* loadmodel(const char* path){	
	...
	m= new model;
	m->position = (_3DVec*)malloc(portion*sizeof(_3DVec));
	m->normal   = (_3DVec*)malloc(portion*sizeof(_3DVec));
	m->texcoord = (_2DVec*)malloc(portion*sizeof(_2DVec));
	...
	while(getline(objfile, line))    //read stream line by line
	{
		...
		if(type == "v"){
			float x, y, z;
			in >> x >> y >> z;      
			m->position[iv++].Set(x,y,-z);
		}
		if(type == "vn"){
			float x, y, z;
			in >> x >> y >> z;      
			m->normal[ivn++].Set(x,y,-z);
		}
		if(type == "vt")				
		{
			float u, v;
			in >> u >> v;      
			m->texcoord[ivt++].Set(u,v);
		}
		if(type == "f"){
			std::string v1,v2,v3;
	...
	for(int i=0;i<iv;i++)
		fwrite(&m->position[i],sizeof(_3DVec),1,cacheFile);
	for(int i=0;i<ivn;i++)
		fwrite(&m->normal[i],sizeof(_3DVec),1,cacheFile);
	for(int i=0;i<ivt;i++)
		fwrite(&m->texcoord[i],sizeof(_2DVec),1,cacheFile);
	face_r tmp;
