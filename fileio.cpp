...
model* loadmodel(const char* path)
{
	...
	model* m;
	bool no_cache = true;
	if (!exists(cachename) || no_cache)
	{

		int portion = 2048 * 128;
		m = new model;
		m->position = (vec3*) malloc(portion * sizeof(vec3));
		m->normal = (vec3*) malloc(portion * sizeof(vec3));
		m->texcoord = (vec2*) malloc(portion * sizeof(vec2));
		...
		while (getline(objfile, line))    //read stream line by line
		{
			...
			if (type == "v")
			{
				float x, y, z;
				in >> x >> y >> z;
				m->position[iv++] = vec3(x, y, -z);
			}
			if (type == "vn")
			{
				float x, y, z;
				in >> x >> y >> z;
				m->normal[ivn++] = vec3(x, y, -z);
			}
			if (type == "vt")
			{
				float u, v;
				in >> u >> v;
				m->texcoord[ivt++] = vec2(u, v);
			}
			if (type == "f")
			{
				std::string v1, v2, v3;
		...
		for (int i = 0; i < iv; i++)
		{
			fwrite(&m->position[i], sizeof(vec3), 1, cacheFile);
		}
		for (int i = 0; i < ivn; i++)
		{
			fwrite(&m->normal[i], sizeof(vec3), 1, cacheFile);
		}
		for (int i = 0; i < ivt; i++)
		{
			fwrite(&m->texcoord[i], sizeof(vec2), 1, cacheFile);
		}
		face_r tmp;
	...
	else
	{
		...
		for (int i = 0; i < iv; i++)
		{
			fread(&m->position[i], sizeof(vec3), 1, cacheFile);
		}
		for (int i = 0; i < ivn; i++)
		{
			fread(&m->normal[i], sizeof(vec3), 1, cacheFile);
		}
		for (int i = 0; i < ivt; i++)
		{
			fread(&m->texcoord[i], sizeof(vec2), 1, cacheFile);
		}
		face_r tmp;
		for (int i = 0; i < iface; i++)
		{
