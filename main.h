...
struct fpixel
{
	...
	inline void operator+=(const fpixel& v)
	{
		r += v.r;
		g += v.g;
		b += v.b;
	...
	inline void operator*=(const fpixel& p)
	{
		r *= p.r;
		g *= p.g;
		b *= p.b;
	...
	void Set(const float& xr, const float& xg, const float& xb)
	{
		r = xr;
		g = xg;
		b = xb;
...
inline fpixel operator*(const float s, const fpixel& a)
{ return fpixel(s * a.r, s * a.g, s * a.b); }

inline fpixel operator*(const fpixel& a, const fpixel& b)
{ return fpixel(b.r * a.r, b.g * a.g, b.b * a.b); }
...
struct model
{
	...
	void setsize(int iv_, int ivt_, int ivn_, int iface_)
	{
		iv = iv_;
		ivt = ivt_;
		ivn = ivn_;
...
struct camera
{
	...
	camera(glm::vec3 position, glm::vec3 rotation): position(position), rotation(rotation)
	{};
};
...
inline bool check(glm::vec3& cp, glm::vec3& v1, glm::vec3& v2, glm::vec3& planev)
{
...
void
createFaceList(glm::vec3& v, glm::vec3& p, int* facelist, int& facelistCount, voxel** voxellist, int& voxellistCount);

void preparemodel(model* m, glm::vec3& boundMax, glm::vec3& boundMin);

intersection findintersection(model* m, glm::vec3 v, glm::vec3& p, int* facelist, int& facelistCount, voxel** voxellist,
                              int& voxellistCount);

inline void transpose(glm::vec3* v1, glm::vec3* v2, glm::vec3* v3)
{
	double tmp = v1->y;
	v1->y = v2->x;
	v2->x = tmp;
